...
**Interfaces:**
- `lib/inboxPurge.js`: `runInboxPurge(deps?) -> { ok, hardDeleted, pressureDeleted }`. Pass (a): hard-delete rows `deleted_at < now() - 30 days` (clamping future `deleted_at` to now — use `least(deleted_at, now())` semantics by also deleting rows where `deleted_at > now()` older than 30 days from `received_at`; simplest correct SQL: delete where `deleted_at is not null and least(deleted_at, now()) < now() - interval '30 days'` — supabase-js can't express `least`, so fetch candidate ids first with two filters, then delete `.in('id', ids)` batched ≤500). Pass (b) byte pressure: for users whose TOTAL `sum(size_bytes)` (live + tombstoned) > 2 × MAX_LIVE_BYTES, delete oldest tombstoned rows until under. `{ error }` per batch (landmine 11).
...
- `inboxData.js` (all on `supabase` from `src/lib/supabase.js`, `{ error }` checked, throws on error): `listMessages({ limit = 100 })` (metadata columns ONLY — id, from_email, from_name, subject, excerpt, received_at, read_at, web_url, unsubscribe_url, auth_results, size_bytes — never html_body/text_body; live rows, received_at desc); `getMessage(id)` (single row incl. bodies); `markRead(id)`; `removeMessage(id)` (UPDATE deleted_at = now ISO); `clearRead()` (UPDATE deleted_at where read_at not null and deleted_at null); `unreadCount()` (`count: 'exact', head: true`).
