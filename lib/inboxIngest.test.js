...
describe('ingestEmail — spec §3 verdict table', () => {
  ...
  it('case 5b: parse succeeds but fromEmail is null (no From header / undisclosed-recipients / empty <>) -> 422 unparseable; insertMessage never called', async () => {
    const repo = makeRepo();
    const parse = vi.fn().mockResolvedValue({ ...PARSED, fromEmail: null });
    const result = await ingestEmail(happyInput(), happyDeps(repo, { parse }));
    expect(result).toEqual({ status: 422, code: 'unparseable' });
    expect(repo.insertMessage).not.toHaveBeenCalled();
  });

  it('case 5c: parse succeeds but fromEmail is an empty string -> 422 unparseable; insertMessage never called', async () => {
    const repo = makeRepo();
    const parse = vi.fn().mockResolvedValue({ ...PARSED, fromEmail: '' });
    const result = await ingestEmail(happyInput(), happyDeps(repo, { parse }));
    expect(result).toEqual({ status: 422, code: 'unparseable' });
    expect(repo.insertMessage).not.toHaveBeenCalled();
  });
  ...
  // The dedupe-precedes-quota ORDERING itself (whether a redelivered
  // duplicate at a full inbox returns 'duplicate' vs 'over_quota') is
  // enforced entirely inside Postgres, by enforce_inbox_quota's dedupe
  // short-circuit (supabase/migrations/20260731_create_inbox.sql) firing
  // before the UNIQUE(user_id, dedupe_key) constraint. That is a live-DB
  // behavior vitest cannot fake — insertMessage below is a mock returning
  // 'duplicate' by fiat, so this test only pins that ingestEmail itself
  // never independently queries quota state before insert (case 14/17
  // territory), not that the DB gets the ordering right. The DB ordering is
  // verified live by scripts/probe-inbox-custody.mjs's dedupe-precedes-quota
  // probe — that probe, not this test, is the real enforcement point.
