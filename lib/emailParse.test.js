...
describe('parseEmail · web version URL', () => {
  ...
  it(
    'completes quickly on a pathological non-matching view label and still finds the real match',
    // Pinned explicitly (not left to vitest's current 5s default): this test
    // discriminates the old O(n^2) pattern (~14.7s on this input) from the
    // bounded one (~6ms) BY timing out. vitest.config.js sets no global
    // testTimeout today, so it's inherited implicitly — a future global bump
    // above ~15s would silently un-discriminate this regression guard
...
describe('parseEmail · unsubscribe URL', () => {
  ...
  it('rejects a bare "https:" entry without the "//" and falls through to mailto (aligned with webUrl\'s https:// requirement)', async () => {
    const p = await parseEmail(
      buildEml({
        headers: [
          'From: bot <bot@example.org>',
          'Subject: Digest',
          'List-Unsubscribe: <https:not-a-real-url>, <mailto:leave@example.org>',
        ],
      }),
    );
    expect(p.unsubscribeUrl).toBe('mailto:leave@example.org');
...
describe('parseEmail · clamps and control characters', () => {
  ...
  it('clamps at a code-point boundary instead of splitting a surrogate pair', async () => {
    // Constructed via String.fromCodePoint, never a raw literal byte —
    // matches the probe script's own emoji convention (📬, U+1F4EC), a
    // 2-code-unit astral character. Placed so the clamp boundary (500) lands
    // exactly mid-emoji under a plain .slice(0, 500): 499 'S' + the emoji's
    // first surrogate unit = 500 UTF-16 units, dropping the second unit and
    // leaving a lone (unpaired) high surrogate at the end.
    const emoji = String.fromCodePoint(0x1f4ec);
    const longSubject = 'S'.repeat(499) + emoji + 'S'.repeat(50);
    const p = await parseEmail(
      buildEml({
        headers: ['From: bot <bot@example.org>', `Subject: ${longSubject}`],
      }),
    );
    // Clamped to 500 CODE POINTS (not UTF-16 units) — the emoji is one code
    // point but two UTF-16 units, so p.subject.length is 501 here; that's
    // correct, not a regression, because the clamp's unit changed from
    // UTF-16 code units to code points precisely to stop splitting pairs.
    expect([...p.subject]).toHaveLength(500);
    // [...s] groups a well-formed surrogate pair into one 2-code-unit
    // element; a lone (split) surrogate survives as its own 1-code-unit
    // element that still matches the surrogate range — that combination is
    // what a split pair looks like and a whole pair never does.
    const hasLoneSurrogate = [...p.subject].some(
      (c) => c.length === 1 && /[\uD800-\uDFFF]/.test(c),
    );
