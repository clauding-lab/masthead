...
import { resolvePremiumSources, fetchPremiumHeadlines, MAX_PREMIUM_IDS } from './premiumService.js';
...
export async function getHeadlinesForSources(requestedSources, { category = null, premium = null, deps = {} } = {}) {
  ...
  const [catalogResult, customResult, premiumResult] = await Promise.all([
    ...
    premium && premium.ids?.length > 0
      ? d
          .resolvePremiumSources(premium.userId, premium.ids)
          .then((defs) => d.fetchPremiumHeadlines(defs))
          // A resolve-step failure (e.g. repo/DB error) must never fail the
          // whole feed (spec §4.2: catalog still served). fetchPremiumHeadlines
          // itself never rejects (per-feed try/catch) — this only guards the
          // resolve step, which can throw before any def is even known.
          .catch(() => ({
            headlines: [],
            stats: ZERO_STATS,
            premiumStatus: premium.ids
              .filter((id) => typeof id === 'string')
              .slice(0, MAX_PREMIUM_IDS)
              .map((id) => ({ id, ok: false, reason: 'unavailable' })),
          }))
      : Promise.resolve({ headlines: [], stats: ZERO_STATS, premiumStatus: [] }),
  ]);
  // Premium never influences the catalog/custom ok↔503 decision (spec §4.2:
  // "catalog still served — never fail the whole feed for premium"); its
  // headlines and stats are merged in only after that decision is made.
  const merged = finalize(catalogResult, customResult);
  const headlines = [...merged.headlines, ...premiumResult.headlines].sort(
    (a, b) => new Date(b.publishedAt) - new Date(a.publishedAt)
  );
  const feedStats = { ...sumStats(merged.feedStats, premiumResult.stats), served: merged.feedStats.served };
  return { headlines, feedStats, status: merged.status, premiumStatus: premiumResult.premiumStatus };
}
