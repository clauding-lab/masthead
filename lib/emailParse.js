...
function cleanText(value, max) {
  ...
  // Code-point-aware clamp: [...stripped] iterates by Unicode code point
  // (not UTF-16 code unit), so the boundary can never land inside a
  // surrogate pair. A plain .slice(0, max) can split an astral character
  // (e.g. an emoji) in half, leaving a lone surrogate — unstorable in
  // Postgres text (22P05) and, downstream, the same infinite-retry class as
  // the null-fromEmail fix above.
...
function pickUnsubscribeUrl(listUnsubscribe) {
  ...
  // 'https://', matching webUrl's requirement below (findViewOnlineAnchor,
  // pickWebUrl) — a bare 'https:' prefix without the slashes is not a valid
  // absolute URL scheme match and used to be accepted here inconsistently.
  return (
