...
function extractThumbnail(item) {
  ...
  // Try content snippet for first image
...
async function fetchScrapeJS(source) {
  if (!source.selectors) return [];
  // Lazy import to avoid loading puppeteer in serverless
  const { scrapeWithJS } = await import('./jsScraper.js');
  const items = await scrapeWithJS(source.url, source.selectors, source.url);
...
export async function fetchFeed(source) {
  ...
  } else if (source.feedType === 'scrape-js') {
    return fetchScrapeJS(source);
...
export async function fetchAllFeeds(sources, { category, source: sourceId } = {}) {
  ...
  // Skip scrape-js sources in serverless (no browser available)
  const isServerless = !!process.env.VERCEL;
  if (isServerless) {
    filtered = filtered.filter((s) => s.feedType !== 'scrape-js');
  }

  // Per-feed timeout of 8s to prevent one slow feed from blocking everything
  const results = await Promise.allSettled(
    filtered.map((s) => withTimeout(fetchFeed(s), 8000))
  );
