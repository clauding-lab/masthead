...
- Mark-read on mount (via `openMessage`). Header: sender, date, "View original" (web_url, new tab), Unsubscribe button when `unsubscribe_url` — label includes target domain (`new URL(u).hostname` for https; 'mail app' for mailto), `target="_blank" rel="noopener noreferrer"`, never auto-fired. Heart button ships in Task 17 (it wires to `saveInboxMessage`, which Task 17 creates — T16 must not import a not-yet-existing function; sequencing fix, T15 close-out).
...
- Delete affordance (T14 review gap-closure — spec §4.2/§6 user remedy the plan had dropped; T20's drive requires "meter falls instantly on delete"): reader header gains a Delete action → store `remove(id)` then navigate back to `/inbox`. No confirm for a single message (optimistic with rollback). The store issues the server tombstone even when the message is absent from the local list (deep-link case — T14 fix round 2 guarantees this).
...
**Files:**
- Modify: `src/lib/library.js` (predicate + seams + `saveInboxMessage`), `src/lib/library.test.js`, `src/lib/sync.js` (nothing — but its round-trip is tested), `src/pages/ReaderPage.jsx` ONLY IF its resolution touches inbox permalinks (it must refuse: add the predicate check at the same spot the premium branch sits), `src/pages/InboxMessagePage.jsx` + its test (ADD the Heart button here — moved from T16, sequencing fix: it wires to `saveInboxMessage`, born in this task)
...
- Clear read (T14 review gap-closure — spec §4.2/§6 bulk remedy, "bulk clear-read frees the meter" in the spec's client test list; the plan had dropped it): a "Clear read messages" row beside the meter, ConfirmSheet-gated (bulk tombstone, no undo) → store `clearRead()`. The store issues the server bulk write regardless of what the 100-row local page shows (T14 fix round 2 guarantees this).
