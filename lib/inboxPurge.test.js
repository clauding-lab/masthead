...
// wrong call shape either. `maxRowsCap`, when set, simulates a PostgREST
// `db-max-rows` configured BELOW SNAPSHOT_PAGE_SIZE: it clips every page
// (even one produced by an explicit `.range()` request) to at most
// `maxRowsCap` rows, so a short page can show up mid-snapshot, not only on
// the trailing page. `failAt` simulates a DB error on the Nth `.from()` call
// (0-indexed, in call order).
function makeFakeTable(seedRows, { failAt = null, maxRowsCap = null } = {}) {
  ...
  async function resolveCall(record) {
    ...
    const rangedPage = record.range
      ? ordered.slice(record.range[0], record.range[1] + 1)
      : ordered.slice(0, SNAPSHOT_PAGE_SIZE);
    // maxRowsCap simulates the project's OWN max-rows override applying on
    // top of an explicit range — PostgREST clips to db-max-rows regardless
    // of what the caller asked for, so a page can come back short even
    // though more rows exist past it.
    const page = maxRowsCap !== null ? rangedPage.slice(0, maxRowsCap) : rangedPage;
...
describe('fetchByteSnapshot', () => {
  it('paginates past a single page so a large snapshot is never silently truncated', async () => {
    ...
    // Cap-agnostic exit (F4): a full page + a short (trailing, 50-row) page
    // + one more empty page confirming genuine exhaustion — the loop no
    // longer treats "shorter than SNAPSHOT_PAGE_SIZE" as proof of the end.
  ...
  // Cap-agnostic regression (fix-wave F4). SNAPSHOT_PAGE_SIZE (1000) is an
  // ASSUMED ceiling — if the project's actual PostgREST max-rows is ever
  // configured below it, every page comes back shorter than
  // SNAPSHOT_PAGE_SIZE, including pages that are NOT the last one. An exit
  // condition of `page.length < SNAPSHOT_PAGE_SIZE` stops after the FIRST
  // short page and silently truncates the snapshot; the only cap-agnostic
  // exit condition is `page.length === 0` (genuinely exhausted), advancing
  // `from` by the page's own length rather than by SNAPSHOT_PAGE_SIZE. This
  // fake's max-rows cap (300) sits below SNAPSHOT_PAGE_SIZE (1000) while the
  // total row count (700) sits above the cap, so a truncating implementation
  // collects only the first capped page and this test fails against it.
  it('yields the complete snapshot even when every page is capped well below SNAPSHOT_PAGE_SIZE (simulated low project max-rows)', async () => {
    const CAP = 300;
    const total = 700;
    expect(CAP).toBeLessThan(SNAPSHOT_PAGE_SIZE);
    expect(total).toBeGreaterThan(CAP);
    const rows = Array.from({ length: total }, (_, i) =>
      row({ id: `c-${String(i).padStart(4, '0')}`, size_bytes: 1 })
    );
    const { client, calls } = makeFakeTable(rows, { maxRowsCap: CAP });
    const snapshot = await fetchByteSnapshot(client);
    expect(snapshot).toHaveLength(total);
    const selectCalls = calls.filter((c) => c.mode === 'select');
    // 300 + 300 + 100 (short, page.length === 0 not yet reached) + 0 (exhausted).
