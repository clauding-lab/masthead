...
export async function fetchByteSnapshot(c) {
  ...
  for (;;) {
    ...
    const page = data || [];
    rows.push(...page);
    // Cap-agnostic: exit ONLY when a page is genuinely empty. SNAPSHOT_PAGE_SIZE
    // is an assumed ceiling, not a guaranteed one — if the project's actual
    // PostgREST max-rows is ever configured below it, every page comes back
    // shorter than SNAPSHOT_PAGE_SIZE, including pages that are NOT the last
    // one. Exiting on `page.length < SNAPSHOT_PAGE_SIZE` would stop after the
    // first such page and silently truncate the snapshot at a different
    // threshold than the one this file was already hardened against.
    // Advancing by the page's own length (not by SNAPSHOT_PAGE_SIZE) keeps
    // `from` correct regardless of how large an actual page came back.
