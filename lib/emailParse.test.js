...
describe('parseEmail · web version URL', () => {
  ...
  // Regression guard for quadratic backtracking: the old pattern
  // (`/view.*(online|browser)/i`) ran O(n^2) against a label this size and
  // took ~940ms at 80K characters, climbing quadratically from there. A
  // massive attacker-controlled label must not make this hang — the test
  // finishing at all (well inside the default test timeout) IS the guard,
  // not a wall-clock assertion.
  it('completes quickly on a pathological view-online label and still finds it', async () => {
    const hugeLabel = `${'view'.repeat(50000)} online`;
    const p = await parseEmail(
      buildEml({
        headers: ['From: bot <bot@example.org>', 'Subject: Digest'],
        contentType: 'text/html; charset=utf-8',
        body: `<p><a href="https://example.org/issue/1">${hugeLabel}</a></p>`,
      }),
    );
