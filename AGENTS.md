...
| Unit tests (vitest, 355 tests incl. `api/**`) | `npm test` |
...
18. **Premium records must NEVER reach the article extractor** (`/api/extract` returns the publisher's public paywall teaser, which would be silently persisted into the user's cloud library as if it were the premium body). The ban is enforced at three independent seams — `resolveReaderSource`'s `'premium'` mode, `retrySave`'s premium branch, and `attachBodyToSaved`'s write-seam guard (all `src/lib/library.js`, keyed off the persisted `savedVia: 'premium'` + `sourceId` marker) — plus ReaderPage's premium-first effect branch. Do not add a new save/reopen/retry path without routing premium records through `fetchPremiumBody`. Test-pinned in `src/lib/library.test.js` with `not.toHaveBeenCalled()` assertions. (Task-review catch, 2026-07-19.)
