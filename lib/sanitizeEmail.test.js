...
describe('sanitizeEmailHtml', () => {
  ...
  describe('Ruling 1 — the five url()-capable style properties never survive', () => {
    ...
    // N3 (T16 re-review): the five pins above pass vacuously if
    // sanitize-html discarded the whole <div> — 'not.toContain(beacon.gif)'
    // is equally true whether the ELEMENT survives with just the offending
    // style property stripped, or the entire element vanished. These
    // positive assertions pin the real claim: the div (and its content)
    // survives sanitisation, and it's specifically the property that's
    // gone, not the element around it.
    it.each([
      ['background-image', 'background-image:url(https://t.test/beacon.gif)'],
      ['background', 'background:url(https://t.test/beacon.gif)'],
      ['list-style-image', 'list-style-image:url(https://t.test/beacon.gif)'],
      ['border-image', 'border-image:url(https://t.test/beacon.gif) 30 fill'],
      ['cursor', 'cursor:url(https://t.test/beacon.gif),pointer'],
    ])('%s: the div element and its content survive — only the property is gone, not the element', (label, styleValue) => {
      const out = sanitizeEmailHtml(`<div style="${styleValue}">x</div>`);
